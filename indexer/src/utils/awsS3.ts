...
export const generateS3Url = (path: string) => `https://${AWS_S3_BUCKET}.s3.${AWS_REGION}.amazonaws.com/${path}`
...
async function getObject(
  ...
  return new Promise<Buffer>(async (resolve, reject) => {
    try {
      ...
      readableBody.on("data", (chunk: Buffer) => responseDataChunks.push(chunk))
      readableBody.once("end", () => resolve(Buffer.concat(responseDataChunks)))
    } catch (err) {
